...
**Feature slice grouping** — `src/features/` nests slices under a domain parent folder (e.g. `src/features/courses/course-detail/`, `src/features/home/top-books/`) whenever more than one slice belongs to the same domain; a domain with only one slice stays flat at `src/features/<slice>/` (e.g. `auth`, `contact`, `live`, `news`, `static-page`). This is purely a directory-organization convention for navigability, borrowed in spirit from Nest's module folders — it is **not** a DI/module boundary and must never grow a barrel `index.ts` at the domain level (that would violate the code-splitting mandate below by letting one slice's bundle pull in its siblings). When a domain's primary slice happens to share the domain's name (e.g. the `ranking` page inside the `ranking` domain, `home` inside `home`, `profile` inside `profile`), nest it the same as any sibling rather than flattening it to the domain root — e.g. `src/features/ranking/ranking/`, not `src/features/ranking/*` directly. When adding a new slice: if it clearly belongs to an existing domain, nest it there; if it's a new domain, decide at scaffold time whether sibling slices are likely (nest under a new domain folder) or it's a one-off (leave it flat).
...
### 3. Ranking (done — `src/features/ranking/ranking`, shared table also used by the Home widget)
...
- [x] Responsive/mobile variants for catalog + detail — confirmed live in a real browser (Playwright, driven against a locally-run backend with seed data) at 375px/390px and both sides of the `lg` (1024px) boundary. Catalog: sidebar correctly hides below `lg` in favor of the Dialog-based mobile filter (same fields as desktop — level/category/language/star-rating — plus "Tozalash"/"Filterni qo'llash"), applying a filter in the dialog fires the expected `GET /courses/read` query and closes cleanly, no horizontal overflow or clipped text at any width. Detail page: found and fixed a real mobile bug — the price/buy-CTA box was DOM-order-last in the `lg:grid-cols-[1fr_360px]` grid, so on mobile (where the grid collapses to one column) it landed below the entire lesson list and full reviews list instead of near the top like on desktop; fixed via explicit `lg:col-start-*`/`lg:row-start-*` grid placement in `course-detail-view.tsx` so mobile DOM order (unaffected by the `lg:`-only placement classes) now stacks image/title/description → price/CTA → sections → reviews, while desktop's two-column layout is pixel-unchanged. Re-verified post-fix at 375px/390px with no overflow/clipping/overlap.

### 6. Library (books) (done — `src/features/library`)
- [x] Catalog (`GET /books/read`) with filters (`GET /books/categories/read`, `GET /authors/read`, `GET /difficulty/read`, `GET /languages/read`) — schemas verified field-for-field against live `/swagger/books-json`. Layout matches Figma: breadcrumb, title box, standalone debounced (700ms) search box, left sidebar Filter panel (star-click rating control instead of a dropdown), vertical row-card list (`book-list-card.tsx` — the grid `book-card.tsx`/equivalent is kept as-is for Home's widgets). Verified end-to-end in a real browser against local seed data.
...
- [x] Responsive variant — confirmed live in a real browser (Playwright, driven against a locally-run backend with the 10 seeded books) at 375px/390px and both sides of the `lg` (1024px) boundary. Catalog: sidebar correctly hides below `lg` in favor of the Dialog-based mobile filter (same fields as desktop — language/difficulty/category/star-rating — plus "Tozalash"/"Filterni qo'llash"), applying a filter (star rating and category) in the dialog fires the expected `GET /books/read` query, shows the active-filter dot on the trigger, and closes cleanly; no horizontal overflow or clipped text at any width. Detail page: checked for the same DOM-order-vs-mobile-position bug just fixed on Courses' detail page — it doesn't apply here, since (unlike course-detail-view.tsx's two-item grid) the book detail page only grids the cover image into a fixed-width column at `lg`; the price/add-to-cart box lives in the same single flex column as the title/author/rating/description on every breakpoint, so it already reflows correctly above the fold on mobile with no separate-`aside` DOM-order trap to fix. Also verified add-to-cart (real `POST /cart/add/{id}` → 201) and the rating widget (only rendered for authenticated users, confirmed via a real registered session) both render without overflow/overlap at 375px. No code changes were needed — the existing implementation held up as-is.
...
### 9. Profile (done — `src/features/profile/profile`)
...
### 10. Cart / Checkout (done — `src/features/commerce/cart`, `src/features/commerce/checkout`)
...
- [x] **Resolved — courses/sign-in duplicates are leftover iterations; the Profile duplicates hid one real, unbuilt state.** Rendered the 3 "Profile" frames (`215:2583`, `1193:336627`, `1188:331227`) as images to check: the first two are an old-vs-new field-set iteration (combined "Ism-sharifingiz" + phone-based security tile vs. the current split "Ism"/"Familiya" + email security tile — the second matches what's actually built). But the **third frame is a genuine, distinct state the other two aren't**: a "Tzimdan chiqish" (log out) confirmation dialog ("Rostan ham tizimdan chiqishni tasdiqlaysizmi?" / Bekor qilish / Davom etish), opened from a red "Chiqish" button living directly on the Profile page's General settings card — a second logout entry point separate from the header dropdown's. **This is currently unbuilt**: `site-header.tsx`'s `UserMenu.handleLogout` (see `src/components/shared/layout/site-header.tsx`) calls `POST /auth/logout` immediately on click with no confirmation step, and `ProfileGeneralSection`/`ProfileHeader` have no logout affordance at all. Backend already supports it (`POST /auth/logout`, already wired) — this is a real, buildable gap, not a backend blocker. The 5 "Education / Courses / Single" duplicates and the 2 "Sign in / Phone number" duplicates were also checked structurally — those are genuinely superseded design-iteration leftovers (the shipped course-detail page's frame is the one with the real reviews/banners sidebar; the phone sign-in frames are moot per the Auth section's backend-driven note below) — no hidden states there.
- [ ] **Still open — needs a product call, not something inferable from Figma/code.** Confirm the production country list for the Ranking filter (curated federation list vs. full FIDE list) — a content decision.
- [x] **Resolved — no interactive/playable chessboard in scope.** No playable/interactive board component exists anywhere in the file; the only "Board" frame found is the decorative one already built into the 404 page. Matches what's already implemented — no action needed.
