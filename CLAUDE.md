...
- [ ] Responsive/mobile variants for catalog + detail — implemented (catalog's filter sidebar now hides below `lg` in favor of a Dialog-based mobile filter matching Figma's dedicated "filter(responsive)" frame; detail page and row cards already used mobile-safe grid/flex patterns) and confirmed lint/typecheck-clean, but still **not confirmed live in a real narrow-viewport browser session** — this environment's dev server couldn't be driven headlessly this pass, so treat the layout as implemented-but-unverified until someone actually opens it at a real narrow width
...
- [ ] Responsive variant — implemented, mirroring the same Figma-backed mobile filter Dialog treatment added to Courses' catalog (see that section's note) for consistency between the two catalogs; lint/typecheck-clean but likewise **not yet confirmed live in a real narrow-viewport browser session**
