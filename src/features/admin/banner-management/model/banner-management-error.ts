...
// `unknown` at the catch site). Duplicated from news-management's identical
// helper (@/features/admin/news-management/model/news-management-error.ts)
// rather than imported — each feature's model layer is self-contained per
// CLAUDE.md's code-splitting mandate.
function getBannerManagementErrorMessage(error: unknown, fallback: string): string {
...
export { getBannerManagementErrorMessage }
