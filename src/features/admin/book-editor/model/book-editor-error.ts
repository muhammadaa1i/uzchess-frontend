...
// `unknown` at the catch site). Duplicated from banner-management's identical
// helper (@/features/admin/banner-management/model/banner-management-error.ts)
// rather than imported — each feature's model layer is self-contained per
// CLAUDE.md's code-splitting mandate.
function getBookEditorErrorMessage(error: unknown, fallback: string): string {
...
export { getBookEditorErrorMessage }
