...
// `unknown` at the catch site). Duplicated from role-management's identical
// helper (@/features/admin/role-management/model/role-management-error.ts)
// rather than imported — each feature's model layer is self-contained per
// CLAUDE.md's code-splitting mandate.
function getNewsManagementErrorMessage(error: unknown, fallback: string): string {
...
export { getNewsManagementErrorMessage }
