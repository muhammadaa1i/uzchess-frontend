...
// `unknown` at the catch site). Duplicated from auth's identical helper
// (@/features/auth/model/auth-error.ts) rather than imported — each
// feature's model layer is self-contained per CLAUDE.md's code-splitting
// mandate.
function getCourseErrorMessage(error: unknown, fallback: string): string {
